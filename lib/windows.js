...
const user32 = new ffi.Library('User32.dll', {
	...
	FindWindowExA: ['pointer', ['pointer', 'pointer', 'pointer', 'pointer']]
});
...
function windows() {
	...
	const omniboxHwndBuffer = Buffer.alloc(pathLengthBytes);

	const omniboxHwnd = user32.FindWindowExA(processHandle,activeWindowHandle, omniboxHwndBuffer, windowTitle)

	console.log("omniboxHwnd", omniboxHwnd)
